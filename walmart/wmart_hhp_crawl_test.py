...
class WalmartIntegratedCrawlerTest:
    ...
    def run(self):
        ...
        print("\n" + "="*80)
        print("Walmart Integrated Crawler (Test Mode)")
        if self.resume_from:
        ...
        try:
            ...
            if not self.resume_from or self.resume_from == 'main':
                ...
                main_crawler = WalmartMainCrawler(test_mode=True, batch_id=self.batch_id)
            ...
            if not self.resume_from or self.resume_from in ['main', 'bsr']:
                ...
                bsr_crawler = WalmartBSRCrawler(test_mode=True, batch_id=self.batch_id)
            ...
            # ========================================
            # STEP 3: Detail 크롤러 실행 (테스트 모드)
            # ========================================
            print("\n" + "="*80)
            print("STEP 3: Detail Crawler (Test Mode - All Products from Main+BSR)")
            print("="*80 + "\n")
            ...
            print("\n" + "="*80)
            print("Walmart Integrated Crawler (Test Mode) - COMPLETED")
            print("="*80)
            print(f"[INFO] Batch ID: {self.batch_id}")
            print(f"[INFO] Start Time: {self.start_time.strftime('%Y-%m-%d %H:%M:%S')}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Walmart HHP Integrated Crawler (Test Mode)',
    ...
    if args.resume_from and not args.batch_id:
        ...
        print("Example: python wmart_hhp_crawl_test.py --resume-from detail --batch-id w_20250123_143045")
        exit(1)

    # 크롤러 실행
    crawler = WalmartIntegratedCrawlerTest(
    ...
    if success:
        print("\n[SUCCESS] Walmart Integrated Crawler (Test Mode) completed successfully")
        exit(0)
    else:
        print("\n[FAILED] Walmart Integrated Crawler (Test Mode) failed")
        exit(1)
